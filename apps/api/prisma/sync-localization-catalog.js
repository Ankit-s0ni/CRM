...
async function main() {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    const stale = await tx.localizationKey.deleteMany({
      where: {
        key: {
          startsWith: 'tenant.ui.',
          notIn: source.entries.map(({ key }) => key),
        },
      },
    });
    deletedStaleKeys = stale.count;
  });

  console.log(
    `Registered ${source.entries.length} English keys and ${translatedCount} Arabic draft translations; removed ${deletedStaleKeys} stale generated keys.`,
  );
}
