...
export class PlatformCatalogController {
  ...
  @RequirePlatformPermissions('platform.modules.read')
  @ApiOperation({
    summary: 'List customer-facing products, add-ons, and capabilities',
  })
  list() {
...
export class PlatformTenantEntitlementsController {
  ...
  @RequirePlatformPermissions('platform.modules.read')
  @ApiOperation({
    summary: 'Get plan, overrides, and effective tenant entitlements',
  })
  list(@Param('tenantId', ParseUUIDPipe) tenantId: string) {
  ...
  replace(
    ...
    return this.modules.replaceTenantCapabilityOverrides(tenantId, dto, actor, {
      ipAddress: request.ip,
      userAgent: request.get('user-agent'),
      requestId: String(request.headers['x-request-id'] ?? ''),
    });
