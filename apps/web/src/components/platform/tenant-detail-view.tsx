...
export function TenantDetailView({ tenantId }: { tenantId: string }) {
  ...
  return <>
    <div className="mx-auto max-w-[1500px] p-5 lg:p-8">
      ...
      <section className="relative mt-5 overflow-hidden rounded-xl border border-red-200 bg-[#fff7f7] p-5"><AlertTriangle className="absolute right-6 top-5 size-16 text-red-100" /><h2 className="font-semibold text-red-700">Danger Zone</h2><p className="mt-1 max-w-2xl text-xs leading-5 text-[#646273]">These actions affect access immediately. Suspended users must authenticate again after reactivation.</p><div className="mt-5 flex flex-wrap items-center justify-between gap-4 rounded-lg border border-red-100 bg-white p-4"><div><div className="text-sm font-semibold">{active ? "Suspend tenant" : "Reactivate tenant"}</div><div className="text-xs text-[#777587]">{active ? "Immediately revoke all user access." : "Restore workspace access without restoring sessions."}</div></div>{canLifecycle && <Button className={active ? "bg-red-600 text-white hover:bg-red-700" : "bg-green-600 text-white hover:bg-green-700"} onClick={() => setLifecycle(active ? "suspend" : "reactivate")}>{active ? "Suspend" : "Reactivate"}</Button>}</div></section>
    </div>
