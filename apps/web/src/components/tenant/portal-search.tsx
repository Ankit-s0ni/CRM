  ...
  return (
    <form className="relative w-full max-w-md" onSubmit={submit} role="search">
      ...
      {open &&
        ...
        <div className="absolute left-0 right-0 top-12 z-50 max-h-96 overflow-y-auto rounded-xl border border-[#ded9e8] bg-white p-2 shadow-xl">
          {visibleEmployees.map((employee) => (
            <button
              className="flex w-full items-center gap-3 rounded-lg p-3 text-left hover:bg-[#f5f1fb]"
              key={employee.id}
              onMouseDown={() => navigate(`/app/employees/${employee.id}`)}
              type="button"
            >
              <span className="grid size-9 place-items-center rounded-lg bg-[#e3e0ff] text-[#3525cd]">
              ...
              <ArrowRight className="size-4 text-[#aaa3cd]" />
            </button>
          ))}
          {matchedDestinations.map(({ label, href }) => (
            <button
              ...
              type="button"
            >
              <span className="grid size-9 place-items-center rounded-lg bg-[#f0ecf9] text-[#3525cd]">
