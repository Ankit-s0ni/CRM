...
export function PlatformDashboard() {
  ...
  const cards = [
    { label: "Monthly recurring revenue", value: billing?.revenueByCurrency.map(({ currency, mrr }) => formatMoney(mrr, currency)).join(" + ") || "No revenue", detail: "Authoritative active per-seat subscriptions", icon: IndianRupee, tone: "text-[#151515] bg-zinc-50" },
    ...
    { label: "Total employees", value: data.metrics.employees.toLocaleString(), detail: `Across ${data.metrics.tenants} workspaces`, icon: Users, tone: "text-[#151515] bg-[#ede7dc]" },
  ...
  return <div className="mx-auto max-w-[1500px] space-y-6 p-5 lg:p-8">
    <div className="flex flex-wrap items-end justify-between gap-4"><div><h1 className="text-3xl font-bold tracking-tight text-[#151515]">Regional Operations Center</h1><p className="mt-1 text-sm text-on-surface-variant">Real-time overview of the DeltCRM ecosystem.</p></div><Link href="/platform/tenants" className="rounded-lg bg-[#151515] px-5 py-2.5 text-sm font-semibold text-white shadow">Manage tenants</Link></div>
