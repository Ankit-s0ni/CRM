...
export function PlatformDashboard() {
  ...
  return <div className="mx-auto max-w-[1500px] space-y-6 p-5 lg:p-8">
    ...
    <div className="grid gap-5 xl:grid-cols-[1.7fr_1fr]">
      ...
      <section className="rounded-xl border border-surface-variant bg-white p-6 shadow-sm"><h2 className="font-semibold">Tenants by plan</h2><p className="text-xs text-outline">Current subscription mix</p><div className="mt-7 space-y-5">{data.planMix.length ? data.planMix.map((plan, index) => <div key={plan.planId}><div className="mb-2 flex justify-between text-sm"><span className="font-medium">{plan.name}</span><span>{plan.tenants}</span></div><div className="h-2 overflow-hidden rounded-full bg-outline-variant"><div className={index % 2 ? "h-full bg-emerald-500" : "h-full bg-[#2a2927]"} style={{ width: `${Math.max(8, plan.tenants / planTotal * 100)}%` }} /></div></div>) : <p className="py-12 text-center text-sm text-outline">No active subscriptions yet.</p>}</div></section>
    </div>
