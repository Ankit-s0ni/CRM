      ...
      <aside className={cn("fixed inset-y-0 left-0 z-50 flex w-[280px] flex-col bg-[#2a2927] text-white shadow-xl transition-transform lg:translate-x-0", mobileOpen ? "translate-x-0" : "-translate-x-full")}>
        ...
        <nav className="space-y-1 px-3 pt-5">
          {navigation
            ...
            return item.enabled ? (
              <Link key={item.label} href={item.href} onClick={() => setMobileOpen(false)} className={cn("flex h-11 items-center gap-4 rounded-lg border-l-4 px-4 text-sm transition", active ? "border-emerald-300 bg-[#2a2927] text-white" : "border-transparent text-indigo-100 hover:bg-white/10 hover:text-white")}>
