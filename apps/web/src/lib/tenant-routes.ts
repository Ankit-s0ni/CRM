...
export function replacePathLanguage(pathname: string, language: AppLanguage) {
  return `/${language}${stripLanguagePrefix(pathname)}`;
}

export function localizedTenantPath(path: string, language: AppLanguage) {
...
export function safeLocalizedNextPath(value?: string | null) {
  ...
  return languageFromPathname(value) &&
    stripLanguagePrefix(value).startsWith("/app")
    ? value
    : null;
}
...
export function resolveTenantLoginDestination(input: {
  ...
  const nextLanguage = safeNext ? languageFromPathname(safeNext) : null;
  const savedLanguage =
    input.savedLanguage === "en" || input.savedLanguage === "ar"
      ? input.savedLanguage
      : null;
  const language =
    nextLanguage && input.enabledLanguages.includes(nextLanguage)
      ? nextLanguage
  ...
  if (
    safeNext &&
    nextLanguage &&
    input.enabledLanguages.includes(nextLanguage)
  ) {
    return safeNext;
  }
