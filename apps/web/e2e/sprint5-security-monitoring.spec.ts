...
test.beforeEach(async ({ page }) => {
  ...
  await page.route("http://localhost:4001/security-alerts?*", (route) => {
    ...
    const matchesStatus = !query.get("status") || query.get("status") === alert.status;
    const matchesType = !query.get("alertType") || query.get("alertType") === alert.alertType;
    const data = matchesStatus && matchesType ? [alert] : [];
    return route.fulfill({
      json: { data, meta: { page: 1, limit: 25, total: data.length, pages: 1 } },
    });
  });
  await page.route(`http://localhost:4001/security-alerts/${alertId}/evidence`, (route) =>
    route.fulfill({
      json: {
        data: {
  ...
  await page.route(`http://localhost:4001/devices/${deviceId}/block`, (route) =>
    route.fulfill({ status: 201, json: { data: { id: deviceId, status: "BLOCKED" } } }),
  );
  for (const action of ["acknowledge", "resolve", "dismiss"] as const) {
    await page.route(`http://localhost:4001/security-alerts/${alertId}/${action}`, (route) => {
      alert = {
        ...alert,
        status:
          action === "acknowledge"
...
test("H14 authorizes private evidence and blocks the implicated device", async ({ page }) => {
  await page.goto("/app/attendance/security");
  await expect(page.getByRole("heading", { name: "Security Monitoring Feed" })).toBeVisible();
  ...
  await expect(page.getByAltText("Authorized attendance attempt evidence")).toBeVisible();
  await expect(page.getByText("Private evidence link expires in 60 seconds.")).toBeVisible();
  await expect(
    page.getByRole("complementary").getByText("2.1 km FROM APPROVED ZONE"),
  ).toBeVisible();

  page.once("dialog", (dialog) => dialog.accept("Root access detected during attendance"));
  ...
  await expect(
    page.getByRole("complementary").filter({ hasText: "Verification evidence" }),
  ).toHaveCount(0);
  await expect(page.getByRole("heading", { name: "Rooted device detected" })).toBeVisible();
});

test("H14 applies event filters", async ({ page }) => {
  await page.goto("/app/attendance/security");
  await page.getByRole("button", { name: "Mock location" }).click();
  await expect(page.getByRole("heading", { name: "No matching security events" })).toBeVisible();
  await page.getByRole("button", { name: /All events/ }).click();
  await expect(page.getByRole("heading", { name: "Rooted device detected" })).toBeVisible();
});

for (const action of ["Acknowledge", "Resolve", "Dismiss"] as const) {
  test(`H14 can ${action.toLowerCase()} an open alert with a note`, async ({ page }) => {
    await page.goto("/app/attendance/security");
    await page.getByRole("button", { name: "View verification log" }).click();
