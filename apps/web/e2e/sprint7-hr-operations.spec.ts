...
test("reviews and approves an attendance correction", async ({ page }) => {
  ...
  await page.route(`**/regularizations/${requestId}`, (route) =>
    route.fulfill({
      json: { data: regularization(approvedComment ? "APPROVED" : "PENDING") },
    }),
  );
  await page.route(`**/regularizations/${requestId}/approve`, async (route) => {
    approvedComment = (route.request().postDataJSON() as { comment: string })
      .comment;
  ...
  await expect(
    page.getByRole("heading", { name: "Correction decision" }),
  ).toBeVisible();
  await page
    .getByLabel("Audit comment")
    .fill("Evidence confirms the missed checkout.");
  await page.getByRole("button", { name: "Approve" }).click();
  await expect
    .poll(() => approvedComment)
    .toBe("Evidence confirms the missed checkout.");
});

test("generates reports and presents an eligible payroll export", async ({
  page,
}) => {
  let generated = false;
  await page.route("**/reports?page=1&limit=100", (route) =>
    route.fulfill({
      json: {
        data: generated ? [{ ...payrollExport(), reportType: "MUSTER" }] : [],
      },
    }),
  );
  await page.route("**/reports/muster", async (route) => {
    generated = true;
    await route.fulfill({
      json: { data: { ...payrollExport(), reportType: "MUSTER" } },
    });
  });
  await page.route(
    "**/reports?reportType=PAYROLL&status=COMPLETED&page=1&limit=100",
    (route) => route.fulfill({ json: { data: [payrollExport()] } }),
  );
  ...
  await expect(
    page.getByRole("heading", { name: "Payroll close" }),
  ).toBeVisible();
...
test("shows leave balances, approval queue, and notification inbox without overflow", async ({
  page,
}) => {
  ...
  await page.route("**/leave-balances/me", (route) =>
    route.fulfill({
      json: {
        data: [{ id: "balance-1", remainingDays: 18.5, policy: leavePolicy() }],
      },
    }),
  );
  ...
  await page.goto("/app/attendance/leave/balances");
  ...
  await page.goto("/app/attendance/leave/approvals");
  await expect(page.getByText("Aisha Employee")).toBeVisible();
...
function regularization(status: "PENDING" | "APPROVED") {
  return {
    ...
    managerComments:
      status === "APPROVED" ? "Evidence confirms the missed checkout." : null,
